...
void handler(int signo)
{
	...
	if (signo==SIGALRM)
	{
		...
		for(int i=0;i<BUF_SIZE;i++)
			encode_buffer[i] = linear2ulaw(record_buffer[i]);

		assert(sizeof(encode_buffer)==sizeof(record_buffer));
...
{
	//handle ctrl + c while closing the connection
